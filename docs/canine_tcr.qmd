...
2. IMGT reference file: This can be downloaded from the `repseqio` repository release: <https://github.com/repseqio/library-imgt/releases>. In the screenshot below, we can see that the IMGT file could be `imgt.202214-2.sv8.json` after decompresssing it.
...
3. The green line is the avg. score at each position, where the orange line are the median (full) and 25% / 75% quantiles (dashed). These lines and the grey shades below 30 indicate thath there are few sequences that need to be trimmed.
...
The clonotypes are assmebled using the command:
...
For this step we suggest to use few seeds, and don't divide the files into too many blocks. Then, to assembly the clonotypes of thse files, we run the saturation module using:
